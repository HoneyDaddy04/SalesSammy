...
router.get("/", async (_req, res) => {
  const sequences = await queryAll(`SELECT * FROM sequences ORDER BY template_key`);
  res.json(sequences);
});

/** GET /api/sequences/:id - get one sequence with parsed touches */
router.get("/:id", async (req, res) => {
  const seq = await queryOne(`SELECT * FROM sequences WHERE id = ?`, [req.params.id]);
...
router.post("/", async (req, res) => {
  ...
  const existing = await queryOne(`SELECT id FROM sequences WHERE template_key = ?`, [key]);
  ...
  const created = await queryOne(`SELECT * FROM sequences WHERE id = ?`, [id]);
...
router.put("/:id", async (req, res) => {
  ...
  await run(`UPDATE sequences SET ${setClauses.join(", ")} WHERE id = ?`, values);

  const updated = await queryOne(`SELECT * FROM sequences WHERE id = ?`, [req.params.id]);
  res.json(updated);
});
...
router.delete("/:id", async (req, res) => {
  ...
  const usage = await queryOne(`SELECT COUNT(*) as c FROM contacts WHERE sequence_id = ?`, [req.params.id]);
  if (usage && (usage.c as number) > 0) {
    res.status(409).json({ error: `Cannot delete: ${usage.c} contacts are using this sequence` });
    return;
  }

  await run(`DELETE FROM sequences WHERE id = ?`, [req.params.id]);
