...
router.get("/tags/all", (req, res) => {
  ...
  const rows = queryAll(`SELECT tags FROM contacts WHERE org_id = ?`, [orgId]);
...
router.post("/webhook", (req, res) => {
  ...
  const resolvedOrgId = org_id || queryOne(`SELECT id FROM organizations LIMIT 1`)?.id as string;

  if (!resolvedOrgId) { res.status(400).json({ error: "No organization found" }); return; }

  const seq = queryOne(`SELECT id FROM sequences WHERE template_key = 'inbound_lead'`);
  const sequenceId = seq?.id as string || null;
  const now = new Date().toISOString();
  let imported = 0;
...
router.put("/:id", (req, res) => {
  ...
  run(`UPDATE contacts SET ${updates.join(", ")} WHERE id = ?`, values);
  res.json(queryOne(`SELECT c.*, s.name as sequence_name FROM contacts c LEFT JOIN sequences s ON s.id = c.sequence_id WHERE c.id = ?`, [req.params.id]));
});

/** POST /api/contacts/:id/pause */
router.post("/:id/pause", (req, res) => {
  run(`UPDATE contacts SET status = 'paused', updated_at = datetime('now') WHERE id = ?`, [req.params.id]);
  res.json({ status: "paused" });
});

/** POST /api/contacts/:id/resume */
router.post("/:id/resume", (req, res) => {
  run(`UPDATE contacts SET status = 'active', next_touch_at = datetime('now'), updated_at = datetime('now') WHERE id = ?`, [req.params.id]);
  res.json({ status: "active" });
});

/** POST /api/contacts/:id/note - add a note */
router.post("/:id/note", (req, res) => {
  const contact = queryOne(`SELECT notes FROM contacts WHERE id = ?`, [req.params.id]) as any;
  if (!contact) { res.status(404).json({ error: "Contact not found" }); return; }
  ...
  run(`UPDATE contacts SET notes = ?, updated_at = datetime('now') WHERE id = ?`, [newNote, req.params.id]);
  res.json({ status: "ok" });
});

/** POST /api/contacts/:id/tag - add/remove tags */
router.post("/:id/tag", (req, res) => {
  const contact = queryOne(`SELECT tags FROM contacts WHERE id = ?`, [req.params.id]) as any;
  if (!contact) { res.status(404).json({ error: "Contact not found" }); return; }
  ...
  run(`UPDATE contacts SET tags = ?, updated_at = datetime('now') WHERE id = ?`, [JSON.stringify(tags), req.params.id]);
  res.json({ tags });
});

/** DELETE /api/contacts/:id */
router.delete("/:id", (req, res) => {
  run(`DELETE FROM touch_queue WHERE contact_id = ?`, [req.params.id]);
  run(`DELETE FROM reply_events WHERE contact_id = ?`, [req.params.id]);
