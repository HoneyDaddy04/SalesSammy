...
router.get("/", (req, res) => {
  ...
  const rows = queryAll(`SELECT * FROM integrations WHERE org_id = ? ORDER BY category, type`, [orgId]);
...
router.post("/connect", (req, res) => {
  ...
  const existing = queryOne(`SELECT id FROM integrations WHERE org_id = ? AND type = ?`, [org_id, type]);

  const encryptedCreds = encrypt(JSON.stringify(credentials || {}));

  if (existing) {
    run(
      `UPDATE integrations SET status = 'connected', credentials = ?, config = ?, last_synced_at = datetime('now') WHERE id = ?`,
    ...
    const id = uuid();
    run(
      `INSERT INTO integrations (id, org_id, type, category, status, credentials, config, last_synced_at) VALUES (?, ?, ?, ?, 'connected', ?, ?, datetime('now'))`,
...
router.post("/test", async (req, res) => {
  ...
  const integration = queryOne(`SELECT * FROM integrations WHERE org_id = ? AND type = ?`, [org_id, type]);
