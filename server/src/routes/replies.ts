...
router.post("/inbound", (req, res) => {
  ...
  if (!contact) {
    ...
    enqueue(org_id, "classify_inbound", {
      messageId: msgId,
      orgId: org_id,
      channel,
  ...
  try { run(`UPDATE contacts SET status = 'replied', updated_at = ? WHERE id = ?`, [now, contactId]); }
  catch { run(`UPDATE contacts SET status = 'replied' WHERE id = ?`, [contactId]); }

  // Enqueue classification job
  enqueue(org_id, "classify_reply", { replyId, contactId, orgId: org_id });
...
router.post("/:id/handle", (req, res) => {
  ...
  const reply = queryOne(`SELECT * FROM reply_events WHERE id = ?`, [req.params.id]);
  if (!reply) { res.status(404).json({ error: "Reply not found" }); return; }

  const now = new Date().toISOString();
  const orgId = reply.org_id as string;
  const contactId = reply.contact_id as string;
  const contact = queryOne(`SELECT name FROM contacts WHERE id = ?`, [contactId]);
  ...
  switch (action) {
    case "respond":
      ...
      run(`UPDATE reply_events SET status = 'handled', routed_action = 'respond' WHERE id = ?`, [req.params.id]);
      break;

    case "escalate":
      run(`UPDATE reply_events SET status = 'escalated', routed_action = 'escalate' WHERE id = ?`, [req.params.id]);
    ...
    case "pause_sequence":
      run(`UPDATE reply_events SET status = 'handled', routed_action = 'pause_sequence' WHERE id = ?`, [req.params.id]);
      run(`UPDATE contacts SET status = 'paused', updated_at = ? WHERE id = ?`, [now, contactId]);
      break;

    case "mark_handled":
      run(`UPDATE reply_events SET status = 'handled', routed_action = 'mark_handled' WHERE id = ?`, [req.params.id]);
      break;
