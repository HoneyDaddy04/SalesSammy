...
router.get("/", (req, res) => {
  ...
  res.json(queryAll(`SELECT * FROM context_overrides WHERE org_id = ? ORDER BY scope_type, scope_id`, [orgId]));
});
...
router.put("/:id", (req, res) => {
  const existing = queryOne(`SELECT * FROM context_overrides WHERE id = ?`, [req.params.id]);
  ...
  run(`UPDATE context_overrides SET ${sets.join(", ")} WHERE id = ?`, vals);
  res.json({ status: "updated" });
});

/** DELETE /api/context-overrides/:id */
router.delete("/:id", (req, res) => {
  run(`DELETE FROM context_overrides WHERE id = ?`, [req.params.id]);
