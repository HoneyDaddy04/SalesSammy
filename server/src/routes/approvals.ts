...
router.post("/:id/resolve", (req, res) => {
  ...
  try {
    const { gate, newValue } = resolveGate(req.params.id, approved, resolved_by || "user");
    ...
    if (approved) {
      ...
      if (gateType === "autonomy_transition" && entityId) {
        ...
        run(`UPDATE teammate SET status = ? WHERE id = ?`, [nv.status, entityId]);
      } else if (gateType === "config_change" && entityId) {
        // Generic config change — apply each key/value to teammate
        for (const [key, value] of Object.entries(newValue)) {
          run(`UPDATE teammate SET ${key} = ? WHERE id = ?`, [typeof value === "object" ? JSON.stringify(value) : value, entityId]);
        }
      }
    }
