...
import { Bot, Send, MessageSquare, ThumbsUp, Clock, CheckCircle2, AlertTriangle, Info, Users, Zap, Loader2, Check, X, ChevronDown, ChevronUp, Edit3 } from "lucide-react";
...
const OverviewView = ({ onViewTeammate }: OverviewViewProps) => {
  ...
  const loadData = async () => {
    ...
    setLoading(true);
    try {
      const [s, q, a, c] = await Promise.all([
  ...
  const handleScan = async () => {
    setScanning(true);
    try { await triggerScan(orgId); await loadData(); } catch {} finally { setScanning(false); }
  };

  const handleApprove = async (id: string) => {
    setActionLoading(id);
    try { await approveTouch(id); await loadData(); } catch {} finally { setActionLoading(null); }
  };

  const handleReject = async (id: string) => {
    setActionLoading(id);
    try { await rejectTouch(id); await loadData(); } catch {} finally { setActionLoading(null); }
  };

  const handleEdit = async (id: string) => {
    if (!editContent.trim()) return;
    setActionLoading(id);
    try { await editTouch(id, editContent); setEditingId(null); await loadData(); } catch {} finally { setActionLoading(null); }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Your AI teammate at a glance</p>
        </div>
        <button onClick={handleScan} disabled={scanning}
          ...
          {scanning ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Zap className="w-3.5 h-3.5" />} Scan for Due Contacts
        </button>
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
        ...
        onClick={onViewTeammate}>
        <div className="flex items-center gap-5">
          <img src={teammateAvatar} alt="Teammate" className="w-16 h-16 rounded-2xl object-cover ring-2 ring-border shadow-md" />
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-1">
              <h3 className="font-display text-xl font-bold text-foreground">Follow-Up Teammate</h3>
